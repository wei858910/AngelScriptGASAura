class UAUW_AttributeMenu : UAuraUserWidget
{
	...
	void OnWidgetControllerSet() override
	{
		...
		for (auto Element : PrimaryAttributeWidgets)
		{
			...
			if (IsValid(WidgetController) && IsValid(WidgetController.AttributeSet))
			{
				auto AttributeMap = WidgetController.AttributeSet.GetAllAttributes();
				if (AttributeMap.Contains(AttributeName))
				{
					float32 AttributeValue = AttributeMap[AttributeName].GetCurrentValue();
